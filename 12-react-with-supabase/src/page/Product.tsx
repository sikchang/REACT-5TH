...
function Product() {
  ...
  /* 
    1. 로그인되지 않은 사용자 => "상품 목록은 로그인 후 이용 가능합니다"
  */
  ...
  /* 
    ...
    2. jsx안에 { 데이터 } 집어넣기
  */
  const handleRoputer = (e:React.MouseEvent<HTMLAnchorElement>,id:string) => {
    e.preventDefault();
    ...
    <div className={S.container}>
      <ul>
        { // map
          ...
          products.map((item) => (
            <li key={item.id}>
            <a href="#" onClick={(e) => handleRoputer(e,item.id)}>
              <figure>
                <img src={item.image_url} alt="상품 사진" />
              </figure>
